...
from mradio.basis.traces_event import Handling3dTracesOfEvent
...
class ZhairesSummaryFileVers28:
    ...
    def __init__(self, file_sry="", str_sry=""):
        ...
        self.l_error = []
        self.d_re = {
            "vers_aires": "This is AIRES version\s+(?P<vers_aires>\w+\.\w+\.\w+)\s+\(",
            "vers_zhaires": "With ZHAireS version (?P<vers_zhaires>\w+\.\w+\.\w+) \(",
            "primary": "Primary particle:\s+(?P<primary>\w+)\s+",
            "site": f"Site:\s+(?P<name>\w+)\s+\(Lat:\s+(?P<lat>{REAL})\s+deg. Long:\s+(?P<lon>{REAL})\s+deg",
            "geo_mag": f"Geomagnetic field: Intensity:\s+(?P<norm>{REAL})\s+(?P<unit>\w+)\s+I:\s+(?P<inc>{REAL})\s+deg. D:\s+(?P<dec>{REAL})\s+deg",
            'energy': f"Primary energy:\s+(?P<value>{REAL})\s+(?P<unit>\w+)",
            'zenith_angle': f"Primary zenith angle:\s+(?P<zenith_angle>{REAL})\s+deg",
            'azimuth_angle': f"Primary azimuth angle:\s+(?P<azimuth_angle>{REAL})\s+deg",
            "x_max": f'Location of max\.\((?P<unit>\w+)\):\s+{REAL}\s+{REAL}\s+(?P<x>{REAL})\s+(?P<y>{REAL})\s+(?P<z>{REAL})\s+',
            "t_sample_ns": f"Time bin size:\s+(?P<t_sample_ns>{REAL})ns"
            }
    ...
    def extract_all(self):
        ...
        for key, s_re in self.d_re.items():
            ...
            if ret:
                ...
                logger.error(f"Can't find '{key}' information with this regular expression:\n{s_re}")
...
class ZhairesSummaryFileVers28b(ZhairesSummaryFileVers28):
    ...
    def __init__(self, file_sry="", str_sry=""):
        ...
        self.d_re["x_max"] = f'Pos. Max.:\s+{REAL}\s+{REAL}\s+(?P<x>{REAL})\s+(?P<y>{REAL})\s+(?P<z>{REAL})\s+'


# add here all version of ZHaireS summary file 
L_SRY_VERS = [ZhairesSummaryFileVers28, ZhairesSummaryFileVers28b]


class ZhairesFromTxtFiles:
    ...
    def read_antpos_file(self):
        a_dtype = {'names':('idx', 'name', 'x', 'y', 'z'), 'formats': ('i4', 'S20', 'f4', 'f4', 'f4',)}
